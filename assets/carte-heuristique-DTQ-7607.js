  ...
  <main class="map-shell" id="mapShell">
    ...
    #metroZone .station.is-sst .station-label {
      ...
      font-weight: 600;
            ...
            ${m}
          </g>`}).join(``);ne.innerHTML=t,x.innerHTML=e.flatMap(({postes:e})=>e.filter(J).map((e,t)=>qe(e,t))).join(``),Qe()}function Qe(){te.setAttribute(`transform`,`translate(${z.x} ${z.y}) scale(${z.k})`);let e=Math.abs(z.x)>.5||Math.abs(z.y)>.5||Math.abs(z.k-1)>.001;n.dataset.resetDisponible=String(e),e!==de&&(de=e,n.dispatchEvent(new CustomEvent(`metro-reset-availability-change`,{detail:{available:e}})))}function $e(){ue||=requestAnimationFrame(()=>{ue=0,Qe()})}function et(e,t,n=w){let r=S.getBoundingClientRect(),i=S.scrollLeft+10,a=S.scrollTop+10,o=S.scrollLeft+e-r.left+14,s=S.scrollTop+t-r.top+14,c=Math.max(i,S.scrollLeft+r.width-n.offsetWidth-10),l=Math.max(a,S.scrollTop+r.height-n.offsetHeight-10);n.style.left=`${Math.max(i,Math.min(o,c))}px`,n.style.top=`${Math.max(a,Math.min(s,l))}px`}function tt(t,n,r){if(!t?.classList.contains(`satellite`))return;rt(),j&&j!==t&&j.classList.remove(`is-sat-hover`),x.querySelectorAll(`.is-sat-parent-highlight`).forEach(e=>e.classList.remove(`is-sat-parent-highlight`)),j=t,t.classList.add(`is-sat-hover`);let i=t.dataset.parentKey;[...x.querySelectorAll(`[data-parent-key]`)].filter(e=>!e.classList.contains(`satellite`)&&e.dataset.parentKey===i).forEach(e=>e.classList.add(`is-sat-parent-highlight`));let a=Object.values(D).flat().find(e=>e.id===t.dataset.nodeId);if(!a)return;let o=K(a),s=Object.values(D).flat().filter(e=>!W(e)&&K(e)===o),c=s.find(e=>e.lineCode===a.lineCode)||s.find(e=>e.sourceLine===a.sourceLine)||s[0],l=le.get(a.id)||{nombreAppareils:0,pk:``},u=c&&le.get(c.id)?.nombreAppareils||0,d=e[a.sourceLine]||`Nom de ligne non renseigné`,f=[a.name,a.type].filter(Boolean).join(` `);w.innerHTML=`<strong>${H(a.sat)} · ${H(f)}</strong><br>${H(a.sourceLine)} · ${H(d)}${l.pk?`<br>${H(l.pk)}`:``}<br>Appareils dans ce SAT : ${l.nombreAppareils}<br>Appareils au poste principal : ${u}`,w.classList.add(`is-visible`),et(n,r)}function $(){j?.classList.remove(`is-sat-hover`),j=null,x.querySelectorAll(`.is-sat-parent-highlight`).forEach(e=>e.classList.remove(`is-sat-parent-highlight`)),w.classList.remove(`is-visible`)}function nt(t,n,r){let i=Object.values(D).flat().find(e=>e.id===t.dataset.nodeId);if(!i||W(i))return;$();let a=[...new Map(Object.values(D).flat().filter(e=>K(e)===K(i)).map(e=>[`${e.sourceLine}:${e.key}`,e])).values()].filter(W),o=a.length,s=a.reduce((e,t)=>e+(le.get(t.id)?.nombreAppareils||0),0),c=le.get(i.id)||{nombreAppareils:0,pk:``},l=c.nombreAppareils+s,u=e[i.sourceLine]||`Nom de ligne non renseigné`,d=o?`<br>SAT associés : ${o}<br>Appareils au poste : ${c.nombreAppareils}<br>Appareils dans les SAT : ${s}<br>Appareils au total : ${l}`:`<br>Appareils au poste : ${c.nombreAppareils}`;T.innerHTML=`<strong>${H([i.name,i.type].filter(Boolean).join(` `))}</strong><br>${H(i.sourceLine)} · ${H(u)}${c.pk?`<br>${H(c.pk)}`:``}${d}`,T.classList.add(`is-visible`),et(n,r,T)}function rt(){T.classList.remove(`is-visible`)}function it(){let e=M;M=null,e?.classList.contains(`satellite`)?$():rt()}function at(e){return e.target instanceof Element?e.target.closest(`.station, .hub.junction[data-node-id]`):null}function ot(e){if(!R||R.pointerId!==e.pointerId)return;let t=Math.hypot(e.clientX-R.clientX,e.clientY-R.clientY),n=R.groupe,r=R.pointerType===`touch`?24:10;R=null,!(t>r||L.size>1||!n?.isConnected)&&(M=n,n.classList.contains(`satellite`)?tt(n,e.clientX,e.clientY):nt(n,e.clientX,e.clientY))}x.addEventListener(`click`,e=>{let t=at(e);if(!t)return;let n=t.getBoundingClientRect(),r=e.clientX||n.right,i=e.clientY||n.top+n.height/2;M=t,t.classList.contains(`satellite`)?tt(t,r,i):nt(t,r,i)}),x.addEventListener(`pointerover`,e=>{let t=e.target.closest(`.station, .hub.junction[data-node-id]`);!t||t.contains(e.relatedTarget)||(t.classList.contains(`satellite`)?tt(t,e.clientX,e.clientY):nt(t,e.clientX,e.clientY))}),x.addEventListener(`pointermove`,e=>{let t=e.target.closest(`.station, .hub.junction[data-node-id]`);t&&et(e.clientX,e.clientY,t.classList.contains(`satellite`)?w:T)}),x.addEventListener(`pointerout`,e=>{let t=e.target.closest(`.station, .hub.junction[data-node-id]`);!t||t.contains(e.relatedTarget)||t!==M&&(t.classList.contains(`satellite`)?$():rt())}),x.addEventListener(`focusin`,e=>{let t=e.target.closest(`.station, .hub.junction[data-node-id]`);if(!t)return;let n=t.getBoundingClientRect();t.classList.contains(`satellite`)?tt(t,n.right,n.top+n.height/2):nt(t,n.right,n.top+n.height/2)}),x.addEventListener(`focusout`,e=>{let t=e.target.closest(`.station, .hub.junction[data-node-id]`);t!==M&&(t?.classList.contains(`satellite`)?$():rt())});function st(){let e=[E,...O.flatMap(e=>e.nodes.filter(e=>!W(e))),...k.filter(e=>!W(e))],t=new Map;return e.forEach(e=>{let n=e===E?E.name:[e.name,e.type].filter(Boolean).join(` `),r=V(n);r&&!t.has(r)&&t.set(r,{node:e,libelle:n})}),[...t.values()].sort((e,t)=>e.libelle.localeCompare(t.libelle,`fr`,{sensitivity:`base`}))}function ct(){return h.filter(e=>!P||e===P).sort((e,t)=>{let n=se===`asc`?1:-1,r=e.localeCompare(t,`fr`,{numeric:!0});return ce===`postes`?(G(e).filter(e=>!W(e)).length-G(t).filter(e=>!W(e)).length)*n||r:r*n}).map(e=>({code:e,postes:me(e)})).filter(({postes:e})=>e.length&&(C.checked||e.some(e=>!W(e))))}n.rechercherPosteMetro=e=>{let t=V(e);if(!t)return P?(P=null,Z(),!1):!1;let r=st().find(({libelle:e})=>V(e)===t);if(!r)return P&&(P=null,Z()),!1;let i=P!==r.node.lineCode;P=r.node.lineCode,N===`ligne`?i&&Z():n.changerDispositionMetro(`ligne`);let a=r.node.detailNodeId||r.node.id,o=[...x.querySelectorAll(`[data-node-id]`)].find(e=>e.dataset.nodeId===a);return o?(x.querySelectorAll(`.is-search-highlight`).forEach(e=>e.classList.remove(`is-search-highlight`)),o.getBoundingClientRect(),o.classList.add(`is-search-highlight`),!0):!1};function lt(e){let t=b.getBoundingClientRect(),n=b.viewBox.baseVal;return{x:n.x+(e.clientX-t.left)*n.width/t.width,y:n.y+(e.clientY-t.top)*n.height/t.height}}function ut(e){return Math.min(4,Math.max(.5,e))}function dt(){let e=b.viewBox.baseVal;return{x:e.x+e.width/2,y:e.y+e.height/2}}function ft(e,t=dt()){let n=z.k,r=ut(n*e);if(Math.abs(r-n)<.001)return;let i=(t.x-z.x)/n,a=(t.y-z.y)/n;z.x=t.x-i*r,z.y=t.y-a*r,z.k=r,$e()}n.zoomerCarteMetro=ft,n.changerDispositionMetro=e=>{![`condense`,`ligne`].includes(e)||e===N||(N=e,z={x:0,y:0,k:1},Z(),n.dispatchEvent(new CustomEvent(`metro-disposition-change`,{detail:{disposition:N}})))},n.changerTriLignesMetro=(e,t)=>{![`numero`,`postes`].includes(e)||![`asc`,`desc`].includes(t)||e===ce&&t===se||(ce=e,se=t,N===`ligne`&&Z())};function pt(){return[...L.values()].slice(0,2)}function mt(){let[e,t]=pt();if(!e||!t)return;F=null,S.classList.remove(`is-panning`);let n=lt({clientX:(e.clientX+t.clientX)/2,clientY:(e.clientY+t.clientY)/2}),r=Math.hypot(t.clientX-e.clientX,t.clientY-e.clientY);I={distance:Math.max(1,r),zoom:z.k,mondeX:(n.x-z.x)/z.k,mondeY:(n.y-z.y)/z.k}}function ht(){let[e,t]=pt();if(!I||!e||!t)return;let n=lt({clientX:(e.clientX+t.clientX)/2,clientY:(e.clientY+t.clientY)/2}),r=Math.hypot(t.clientX-e.clientX,t.clientY-e.clientY),i=ut(I.zoom*r/I.distance);z.x=n.x-I.mondeX*i,z.y=n.y-I.mondeY*i,z.k=i,$e()}return b.addEventListener(`pointerdown`,e=>{let t=at(e);if(!t&&e.button===0&&M&&it(),R=t&&e.button===0?{pointerId:e.pointerId,pointerType:e.pointerType,clientX:e.clientX,clientY:e.clientY,groupe:t}:null,N===`ligne`){if(e.pointerType!==`touch`)return;L.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY}),L.size>=2&&(e.preventDefault(),mt(),b.setPointerCapture(e.pointerId));return}if(e.preventDefault(),document.getSelection()?.removeAllRanges(),e.pointerType===`touch`&&(L.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY}),L.size>=2)){mt(),b.setPointerCapture(e.pointerId);return}let n=lt(e);F={x:n.x-z.x,y:n.y-z.y},S.classList.add(`is-panning`),b.setPointerCapture(e.pointerId)}),b.addEventListener(`pointermove`,e=>{if(N===`ligne`){e.pointerType===`touch`&&L.has(e.pointerId)&&(L.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY}),I&&L.size>=2&&(e.preventDefault(),ht()));return}if(e.pointerType===`touch`&&L.has(e.pointerId)&&(L.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY}),I&&L.size>=2)){ht();return}let t=lt(e);F&&(z.x=t.x-F.x,z.y=t.y-F.y,$e())}),[`pointerup`,`pointercancel`,`pointerleave`].forEach(e=>b.addEventListener(e,t=>{if(e===`pointerup`&&ot(t),e!==`pointerup`&&(R=null),t.pointerType===`touch`&&L.delete(t.pointerId),I){I=null,F=null,S.classList.remove(`is-panning`),L.size>=2&&mt();return}F=null,S.classList.remove(`is-panning`)})),b.addEventListener(`wheel`,e=>{N===`ligne`&&!e.ctrlKey&&!e.metaKey||(e.preventDefault(),document.getSelection()?.removeAllRanges(),ft(e.deltaY<0?1.15:.87,lt(e)))},{passive:!1}),b.addEventListener(`selectstart`,e=>e.preventDefault()),b.addEventListener(`dragstart`,e=>e.preventDefault()),document.getElementById(`resetLayout`).addEventListener(`click`,()=>{z={x:0,y:0,k:1},Z()}),window.addEventListener(`resize`,()=>{N===`ligne`&&We()}),document.getElementById(`defaultView`).addEventListener(`click`,()=>{C.checked=!1,n.classList.add(`satellites-hidden`),z={x:0,y:0,k:1},Ae(),Z()}),C.addEventListener(`change`,()=>{$(),n.classList.toggle(`satellites-hidden`,!C.checked),N===`ligne`&&We()}),Promise.all([fetch(`./carte-heuristique.json`,{cache:`no-store`}).then(e=>e.ok?e.json():null).catch(()=>null),fetch(`./postes.geojson`,{cache:`no-store`}).then(e=>{if(!e.ok)throw Error(`postes.geojson (${e.status})`);return e.json()})]).then(([e,t])=>{let r=Array.isArray(e?.lines),i=r?e.lines:[];r&&(h=i.map(e=>B(e.code)).filter(Boolean),m=Object.fromEntries(i.map(e=>[B(e.code),Array.isArray(e.postes)?e.postes:[]])));let a=[...new Set([...Object.keys(p),...h])];D=Object.fromEntries(a.map(e=>[e,ke(t,e)])),Ae(),Z(),je().then(()=>{N===`ligne`?Z():Xe()}).catch(e=>console.warn(`Chargement des détails PK/appareils impossible`,e)),re.textContent=`${h.reduce((e,t)=>e+G(t).length,0)} postes · ordre manuel conservé`,n.dispatchEvent(new CustomEvent(`metro-postes-prets`,{detail:{postes:st().map(({libelle:e})=>e)}}))}).catch(e=>{console.error(e),re.textContent=`Impossible de charger les postes`,x.innerHTML=`<text class="error" x="1600" y="750">Chargement impossible</text>`})})()}export{n as initialiserCarteMetro};